...
![image](https://user-images.githubusercontent.com/55370336/109517718-85bfbe00-7ae4-11eb-89cf-d7881b9a40cf.png)

仍然以人物前景分割为例，如下图，其IoU的计算就是使用intersection / union

|                            target                            |                          prediction                          |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image-20210116105729145](https://github.com/datawhalechina/team-learning-cv/blob/master/AerialImageSegmentation/img/Task4：评价函数与损失函数_image/target.png) | ![image-20210116105742006](https://github.com/datawhalechina/team-learning-cv/blob/master/AerialImageSegmentation/img/Task4：评价函数与损失函数_image/prediction.png) |
|                  Intersection( T ∩ P)                   |                      union(T ∩ P)                       |
| ![image-20210115230310670](https://github.com/datawhalechina/team-learning-cv/blob/master/AerialImageSegmentation/img/Task4：评价函数与损失函数_image/intersection.png) | ![image-20210115230326073](https://github.com/datawhalechina/team-learning-cv/blob/master/AerialImageSegmentation/img/Task4：评价函数与损失函数_image/union.png) |
...
![image](https://user-images.githubusercontent.com/55370336/109518421-3037e100-7ae5-11eb-8b8b-80fc63f43a13.png)

式中，y = {y1, y2,...,yc}，其中yi是非0即1的数字，代表了是否属于第i类，为真实值；hat yi代表属于第i类的概率，为预测值。可以看出，交叉熵损失考虑了多类别情况，针对每一种类别都求了损失。针对二分类问题，上述公式可以改写为：

![image](https://user-images.githubusercontent.com/55370336/109518639-62e1d980-7ae5-11eb-9d58-42dfcc5aae75.png)

式中,y为真实值，非1即0；hat y为所属此类的概率值，为预测值。这个公式也就是BCE损失函数，即二分类任务时的交叉熵损失。值得强调的是，公式中的hat y为概率分布形式，因此在使用BCE损失前，都应该将预测出来的结果转变成概率值，一般为sigmoid激活之后的输出。
...
![image](https://user-images.githubusercontent.com/55370336/109518762-87d64c80-7ae5-11eb-9888-91b5a0058c91.png)

仔细观察就不难发现，它其实是BCE扩展而来，对比BCE其实就多了个

![image](https://user-images.githubusercontent.com/55370336/109518816-93c20e80-7ae5-11eb-9a10-1b8f82734068.png)

为什么多了这个就能缓解正负样本不均衡的问题呢？见下图：

![image-20210202220206367](https://github.com/datawhalechina/team-learning-cv/raw/master/AerialImageSegmentation/img/Task4：评价函数与损失函数_image/FocalLoss.png)
...
首先明确定义，在语义分割任务中，给定真实像素标签向量y*和预测像素标签hat{y}}，则所属类别c的IoU（也称为Jaccard index）如下，其取值范围为`[0,1]`，并规定`0/0=1`：

![image](https://user-images.githubusercontent.com/55370336/109518984-c4a24380-7ae5-11eb-836f-00537660777c.png)

则Jaccard loss为：

![image](https://user-images.githubusercontent.com/55370336/109519025-cff56f00-7ae5-11eb-8014-0621b047c68d.png)

针对类别c，所有未被正确预测的像素集合定义为：

![image](https://user-images.githubusercontent.com/55370336/109519103-e3083f00-7ae5-11eb-96c1-04032cef6894.png)

则可将Jaccard loss改写为关于$M_c$的子模集合函数（submodular set functions）：

![image](https://user-images.githubusercontent.com/55370336/109519117-e8658980-7ae5-11eb-96ed-0af0881de432.png)

方便理解，此处可以把![image](https://user-images.githubusercontent.com/55370336/109519351-1cd94580-7ae6-11eb-8e8d-4e03b7f3036f.png)理解成如图像mask展开成离散一维向量的形式。

![image](https://user-images.githubusercontent.com/55370336/109519647-61fd7780-7ae6-11eb-83b8-98f8ca4eb8ae.png)
